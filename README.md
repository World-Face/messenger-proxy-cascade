...
`mtg doctor` на выходном сервере покажет два ❌ — оба ожидаемы для каскадной схемы, а не поломка:

| Проверка | Почему падает |
|---|---|
| **SNI-DNS match** | `doctor` рассчитан на односерверную установку и ждёт, что домен резолвится в IP выходного сервера. В каскаде домен обязан смотреть на входной — так и задумано |
| **fronting domain connectivity** | На `:443` входного сервера ничего не слушает, поэтому камуфляж mtg для неудачных handshake не сработает. На стойкость к блокировкам не влияет: секрет всё равно требуется, но активный сканер получит закрытое соединение вместо правдоподобного TLS-ответа |
